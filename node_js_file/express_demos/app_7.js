...
// dev is preset in morgan (particular output template)
// which logs , few things on response
app.use(logger('dev'));
...
const getAFruit = (req, res) => {
  ...
  res.status(200).json({ status: "success", data: { fruit } , requestedAt: req.requestTime });
};
...
app.route("/api/v1/users/:id").get(getAUser);
