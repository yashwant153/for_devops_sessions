...
app.post('/signup', async (req, res) => {
  ...
  console.log(`signup::entered in user_app with email ${email} and password ${password}`);
  ...
  try {
    console.log('signup:: calling auth_app');
    const hashedPW = await axios.get(`http://${process.env.AUTH_ADDRESS}/hashed-password/` + password);
    console.log(`signup::inside user_app, got hashedPW `);
...
app.post('/login', async (req, res) => {
  ...
  const email = req.body.email;
  const password = req.body.password;
  console.log(`login::entered in user_app with email ${email} and password ${password}`);
  ...
  const response = await axios.get(
   ...
   console.log('signup::back to  auth_app');
